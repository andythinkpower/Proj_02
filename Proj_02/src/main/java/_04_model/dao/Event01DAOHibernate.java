...
public class Event01DAOHibernate implements Event01DAO {
	...
	public List<Event01> eventSearch(String newDate, String newAreas, String newTypes, String newOrder, Integer pageNumber) {
		Query<Event01> query = this.getSession().createQuery("FROM Event01 " + newDate + " AreaID like :areas and EventTypeID like :types and datediff( day , getdate() , DurationEnd ) >= 0 and CityID = '臺北市' order by " + newOrder + "",	Event01.class);
	...
	public Long eventCount(String newDate, String newAreas, String newTypes) {
		Query query = this.getSession().createQuery("select count(*) FROM Event01 " + newDate + " AreaID like :areas and EventTypeID like :types and datediff( day , getdate() , DurationEnd ) >= 0 and CityID = '臺北市'", Long.class);
		query.setParameter("areas", "%[" + newAreas + "]%");
		query.setParameter("types", "%[" + newTypes + "]%");
